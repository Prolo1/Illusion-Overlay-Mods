...
namespace KoiSkinOverlayX
{
    ...
    {
        private const string DataMarker = "TexEntryID-";
        ...
        public void Load(PluginData data)
        {
            ...
            lock (_data)
            {
                foreach (var dataPair in data.data.Where(x => x.Key.StartsWith(DataMarker)))
                {
                    ...
                    {
                        Console.WriteLine($"Invalid ID {idStr} in key {dataPair.Key}");
                        continue;
                    }
                    ...
                    {
                        Console.WriteLine(
                            $"Invalid value of ID {id}. Should be byte[] but it {dataPair.Value.GetType()}");
                        continue;
                    }
        ...
        public void ClearAndLoad(PluginData data)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));
            Clear();
            Load(data);
        ...
        public int StoreTexture(byte[] tex)
        {
            ...
            lock (_data)
            {
                ...
                for (var i = Random.Range(1000, 9999);; i++)
                {
        ...
        // Automatically recompress? don't give the option at all?
        // todo remove?
        ...
        public Texture2D GetSharedTexture(int id)
        {
            ...
            Console.WriteLine("Tried getting texture with nonexisting ID " + id);
            return null;
        }

        // remember to destroy! note
        // todo remove?
        public Texture2D GetTextureCopy(int id)
        {
            var t = GetSharedTexture(id);
            return t?.TextureToTexture2D();
        }

        public void PurgeUnusedAndWrite(PluginData data, IEnumerable<int> usedIDs)
        {
            PurgeUnused(usedIDs);
            Write(data);
        }
        ...
        private sealed class TextureHolder : IDisposable
        {
            ...
            public Texture2D Texture
            {
                get
                {
                    if (_texture == null)
                        if (_data != null)
                            _texture = Util.TextureFromBytes(_data,
                                KoiSkinOverlayMgr.GetSelectedOverlayTexFormat(false));
                    return _texture;
                }
            }
